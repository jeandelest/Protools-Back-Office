...
class PlatinePilotageGetSUIsToFollowUpTaskTest extends TestWithContext {
    ...
    void execute_should_work_when_context_and_variables_OK() {
        ...
        Boolean isToFollowUp = Boolean.FALSE;
        doReturn(isToFollowUp).when(platinePilotageService).isToFollowUp(any(),any());
        ...
        //Post conditions :
        // 1 : Exactly one call to post platinePilotageService.isToFollowUp with correct parameters
        verify(platinePilotageService, times(1)).isToFollowUp(suId, platinePartionId);
        //2 : VARNAME_SU_IS_TO_FOLLOWUP has been set to correct value
        verify(executionParent, times(1)).setVariableLocal(VARNAME_SU_IS_TO_FOLLOWUP,isToFollowUp);
    }
}